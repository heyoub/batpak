...
Cold start prefers a verified mmap snapshot (`index.fbati`), then checkpoint restore, then full replay. Current fast-start artifacts carry receipt-extension maps directly; older artifacts hydrate those maps from authoritative `.fbat` frames. Cancelled visibility-fence ranges are persisted separately so "durable but hidden" survives reopen and snapshot export.
