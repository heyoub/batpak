...
Correct API: `cargo xtask platform doctor`, `probe`, `verify`, `bless`, and
`audit`.

Minimal code is mirrored by `templates/minimal-store`.

Wrong tempting move: hard-code host assumptions into public substrate APIs.

Test command: `cargo xtask platform audit`.
