...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Canonical encoding contract | `batpak::canonical` (`crates/core/src/encoding.rs`) | compliance ledger, signed audit export | Named-field MessagePack helpers exist and are used in signing/encoding paths | Explicit compatibility contract boundary | ADR defining compatibility promises and non-promises | Low | docs only |
| Schema/fixture snapshot reports | `SchemaSnapshotEvidenceReport` (`crates/core/src/schema.rs`) | data emitter, regulated audit harness | Deterministic schema/fixture drift report contract exists | Optional derive/helper ergonomics only | Keep core report stable; add tooling helpers only when repeated callers need them | Medium | shipped report API |
...
| Canonical artifact envelope | `CanonicalArtifactEnvelope<T>` (`crates/core/src/artifact.rs`) | signed artifact archive, plugin package manager | Generic body-vs-envelope identity model exists | Multi-version envelope verification policy | Keep v1 framing stable; extend by ADR/schema version | High | shipped core primitive |
...
| Deterministic phase cache | existing deterministic crates/core/tests/fixtures; no public phase cache API | simulation pipeline, deterministic build graph | Determinism discipline exists in crates/core/tests/tooling | Generic phase cache contract | Tooling/consumer owner until a content-addressed phase executor exists | Medium | tooling/consumer owner |
