...
pub fn run() -> Result<[u8; 32], Box<dyn std::error::Error>> {
    ...
    // Seed one event. This template demonstrates read-walk evidence, so the
    // append receipt is intentionally discarded here; the `minimal-store`
    // template shows using the receipt's `event_id`.
    let _ = store.append(&coord, EventKind::custom(0xA, 3), &serde_json::json!({ "n": 1 }))?;
