...
const WORKSPACE_LIB_DIR: &str = "/workspace/batpak/bpk-lib";
...
fn exec_in_devcontainer(args: &[String]) -> Result<()> {
    ...
    ensure_devcontainer_image(project_root)?;
    ...
    command
        .arg("-v")
        .arg(format!("{}:{WORKSPACE_DIR}", project_root.display()))
        .arg("-w")
        .arg(WORKSPACE_LIB_DIR)
        .arg(&image);
