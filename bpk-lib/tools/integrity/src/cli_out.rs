//! CLI output macros for the batpak-integrity developer tool.
//!
//! `batpak-integrity` is the repository-owned command-line tool; its check
//! subcommands report human and CI status messages to stdout/stderr. Rather
//! than lean on the `clippy::print_stdout` / `clippy::print_stderr` lints being
//! globally silenced, every call site goes through these macros, which are
//! nothing more than ergonomic `writeln!`/`write!` to a freshly locked standard
//! handle. The denied `print*!` macros are therefore never used anywhere in the
//! crate.
//!
