//! PROVES: S12-SUBSCRIPTION-RUNTIME-ENTITY netbat TCP/stream adaptation.
...
use netbat as nb;
use syncbat::{
    EntityStreamCursorV1, RuntimeCursor, SessionControl, SessionDelivery, SessionEnd, SessionError,
...
#[test]
fn stream_runtime_entity_tcp_maps_entity_sub_event_watermark_and_ack_wake(
    ...
    let server = thread::Builder::new()
        .name("netbat-test-entity-replay-live".to_owned())
        .spawn(move || {
...
#[test]
fn stream_runtime_entity_unknown_subscription_emits_sub_err(
) -> Result<(), Box<dyn std::error::Error>> {
    let runtime = FakeRuntime::unknown();
    let listener = localhost_listener()?;
    let addr = listener.local_addr()?;
    let server = thread::Builder::new()
        .name("netbat-test-entity-unknown".to_owned())
        .spawn(move || {
            let (stream, _) = listener.accept()?;
            let reader = stream.try_clone()?;
            nb::serve_subscription_stream(reader, stream, &runtime, &nb::Limits::default())
...
#[test]
fn stream_runtime_entity_slow_consumer_emits_sub_err() -> Result<(), Box<dyn std::error::Error>> {
    let runtime = FakeRuntime::deliver(vec![slow_consumer_error()], None);
    let listener = localhost_listener()?;
    let addr = listener.local_addr()?;
    let server = thread::Builder::new()
        .name("netbat-test-entity-slow".to_owned())
        .spawn(move || {
            let (stream, _) = listener.accept()?;
            let reader = stream.try_clone()?;
            nb::serve_subscription_stream(reader, stream, &runtime, &nb::Limits::default())
...
#[test]
fn stream_runtime_entity_client_disconnect_without_cancel_exits_server(
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = thread::Builder::new()
        .name("netbat-test-entity-disconnect".to_owned())
        .spawn(move || {
...
fn event_delivery(
    ...
    SessionDelivery::Event(SessionEventDelivery {
        ...
        envelope_bytes: b"canonical-entity-envelope-fixture".to_vec(),
    })
}
...
fn cursor_beginning() -> RuntimeCursor {
    RuntimeCursor::from_bytes(
        EntityStreamCursorV1::beginning(SUBSCRIPTION_ID, ENTITY, SCOPE)
            .encode()
            .to_vec(),
    )
}
...
fn cursor_after(global_sequence: u64) -> RuntimeCursor {
    RuntimeCursor::from_bytes(
        EntityStreamCursorV1::after_global_sequence(
            SUBSCRIPTION_ID,
            ENTITY,
            SCOPE,
            global_sequence,
