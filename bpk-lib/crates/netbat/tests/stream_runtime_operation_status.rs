//! PROVES: S12-SUBSCRIPTION-RUNTIME-OPERATION-STATUS netbat TCP/stream adaptation.
...
use netbat as nb;
use syncbat::{
    OperationStatusStreamCursorV1, RuntimeCursor, SessionControl, SessionDelivery, SessionEnd,
...
const ENTITY: &str = "syncbat:operation-status:mod.a.echo";
...
#[test]
fn stream_runtime_operation_status_tcp_maps_snapshot_watermark_and_ack_wake(
    ...
    let server = thread::Builder::new()
        .name("netbat-test-status-replay-live".to_owned())
        .spawn(move || {
...
#[test]
fn stream_runtime_operation_status_unknown_subscription_emits_sub_err(
) -> Result<(), Box<dyn std::error::Error>> {
    let runtime = FakeRuntime::unknown();
    let listener = localhost_listener()?;
    let addr = listener.local_addr()?;
    let server = thread::Builder::new()
        .name("netbat-test-status-unknown".to_owned())
        .spawn(move || {
            let (stream, _) = listener.accept()?;
            let reader = stream.try_clone()?;
            nb::serve_subscription_stream(reader, stream, &runtime, &nb::Limits::default())
...
#[test]
fn stream_runtime_operation_status_slow_consumer_emits_sub_err(
) -> Result<(), Box<dyn std::error::Error>> {
    let runtime = FakeRuntime::deliver(vec![slow_consumer_error()], None);
    let listener = localhost_listener()?;
    let addr = listener.local_addr()?;
    let server = thread::Builder::new()
        .name("netbat-test-status-slow".to_owned())
        .spawn(move || {
            let (stream, _) = listener.accept()?;
            let reader = stream.try_clone()?;
            nb::serve_subscription_stream(reader, stream, &runtime, &nb::Limits::default())
...
#[test]
fn stream_runtime_operation_status_client_disconnect_without_cancel_exits_server(
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = thread::Builder::new()
        .name("netbat-test-status-disconnect".to_owned())
        .spawn(move || {
...
fn event_delivery(
    ...
    SessionDelivery::Event(SessionEventDelivery {
        ...
        envelope_bytes: b"canonical-operation-status-envelope-fixture".to_vec(),
    })
}
...
fn cursor_beginning() -> RuntimeCursor {
    RuntimeCursor::from_bytes(
        OperationStatusStreamCursorV1::beginning(SUBSCRIPTION_ID, OPERATION, ENTITY)
            .encode()
            .to_vec(),
    )
}
...
fn cursor_after(entity_generation: u64) -> RuntimeCursor {
    RuntimeCursor::from_bytes(
        OperationStatusStreamCursorV1::after_entity_generation(
            SUBSCRIPTION_ID,
            OPERATION,
            ENTITY,
            entity_generation,
