...
impl Reader {
        ...
        F: FnMut(ScannedIndexEntry) -> Result<(), StoreError>,
    {
        ...
        // clamp `frames_end` down to where they actually stop, so cold start
        // recovers every CRC-valid frame instead of parsing footer bytes as frame
        // headers and FailClosing. Recover-what-was-found: over-reading into a
        // corrupt footer is expected, so a decode failure on the way to the hint is
        // the clean end of real frames, not a hard error.
