...
fn try_load_mmap_index(
    ...
    let version = format::read_le_u16(&prefix[6..8])?;
    ...
    let expected_crc = format::read_le_u32(&prefix[8..12])?;
    let mut cursor = 0usize;
    let watermark_segment_id = format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?;
    cursor += 8;
    let watermark_offset = format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?;
    cursor += 8;
    let stored_allocator = format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?;
    cursor += 8;
    let interner_count = format::read_le_u32(header_tail.get(cursor..cursor + 4)?)?;
    cursor += 4;
    let entry_count = format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?;
    cursor += 8;
    let interner_bytes_len = format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?;
    cursor += 8;
    let summary_bytes_len = if version == 1 {
        0usize
    } else {
        usize::try_from(format::read_le_u64(header_tail.get(cursor..cursor + 8)?)?).ok()?
    };
    if version != 1 {
        cursor += 8;
    }
