...
//!
//! PROVES: the raw-msgpack and json-value replay lanes converge on the same
//! `(generation, folded state)` pair for `project` and `watch_projection`,
//! and that `watch_projection` matches `project_if_changed` after both
//! relevant and irrelevant appends.
//! CATCHES: replay-lane drift, watcher/project divergence, and irrelevant
//! appends that wrongly mutate the watched folded state.
