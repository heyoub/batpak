...
/// The structural bounds the validator (step 3) enforces. Frozen FORM — a
/// polynomial node bound and a logarithmic depth bound — with the concrete
/// constants pinned here. The bounds are what make the NC¹ claim checkable rather
/// than rhetorical: a program over its limits is rejected.
...
mod admission_tests {
    use super::{
        ...
        Outputs, ProgramError, Width, ADMISSION_PROGRAM_SCHEMA_VERSION, FROZEN_LIMITS, MAX_WIDTH,
    };
    ...
    #[test]
    fn frozen_limits_admit_a_small_program_and_bound_the_form() {
        let program = budget_compare_program();
        assert!(FROZEN_LIMITS.check(&program).is_ok());
        // The bounds have the frozen form: polynomial nodes, log depth.
        let width = program.max_width();
        assert_eq!(
