...
pub(crate) fn wire_axes_to_region(
    ...
    if let Some(entity) = entity {
        Coordinate::new(entity, "hbat-wire-region").map_err(|error| {
            WireRegionError::InvalidCoordinate {
                field: "entity",
    ...
    region = match (kind_category, kind_type_id) {
        ...
        (Some(category), None) => {
            return Err(WireRegionError::InvalidKind {
                field: "kind_category",
    ...
    match (start_clock, end_clock) {
        (Some(start), Some(end)) => {
            if start > end {
                return Err(WireRegionError::InvalidClockRange {
        ...
        (None, None) => Ok(region),
        _ => Err(WireRegionError::InvalidClockRange {
