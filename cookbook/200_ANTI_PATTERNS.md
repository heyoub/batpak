...
Agent surface task: every task in `bpk-lib/traceability/agent_surface.yaml`.
...
Detector: `just structural`, `cd bpk-lib && cargo xtask agent-doctor`.
...
Detector: evidence report tests and `cd bpk-lib && cargo xtask evidence-audit`.
...
Detector: `cd bpk-lib && cargo xtask agent-doctor`.
