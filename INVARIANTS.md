...
| `INV-SYNCBAT-EFFECT-ROW-ENFORCEMENT` | syncbat operation descriptors declare an effect row consistent with their EffectClass (validated at registration); cross-module effect conflicts are rejected at composition; and operations append events only through a Ctx capability handle that performs the append through the runtime-owned effect backend and records it in the same step, so the observed appends are authoritative. checkout fails closed with a denied receipt when an operation appends an event kind outside its declared row, and an append with no backend bound fails closed rather than reaching a store the runtime did not mediate. |
