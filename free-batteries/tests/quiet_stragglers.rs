...
fn event_kind_system_constants_are_system() {
    ...
    for kind in system_kinds {
        assert!(
            kind.is_system(),
            "EventKind {:?} should be is_system(). Investigate: src/event/kind.rs",
            kind
        );
...
fn event_kind_effect_constants_are_effect() {
    ...
    for kind in effect_kinds {
        assert!(
            kind.is_effect(),
            "EventKind {:?} should be is_effect(). Investigate: src/event/kind.rs",
            kind
        );
...
fn event_is_genesis_true_when_prev_hash_zero() {
    ...
    assert!(
        event.is_genesis(),
        "Event with zero prev_hash should be genesis."
    );
}
...
fn event_is_genesis_false_when_prev_hash_nonzero() {
    ...
    assert!(
        !event.is_genesis(),
        "Event with non-zero prev_hash should not be genesis."
    );
}
...
fn event_is_genesis_true_when_no_hash_chain() {
    ...
    assert!(
        event.is_genesis(),
        "Event without hash_chain should be treated as genesis."
    );
}
...
fn event_map_payload_transforms_preserving_header() {
    ...
    assert_eq!(
        ...
        "map_payload should preserve header."
    );
...
fn dag_position_is_ancestor_of_same_lane() {
    ...
    assert!(
        !b.is_ancestor_of(&a),
        "child(5) should NOT be ancestor of child(2)."
    );
}
...
fn outcome_error_display() {
    ...
    assert!(
        s.contains("Conflict"),
        "Display should contain kind. Got: {s}"
    );
    assert!(
        s.contains("double booking"),
        "Display should contain message. Got: {s}"
    );
}
...
fn denial_serialize() {
    ...
    assert!(
        ...
        "Serialized Denial should contain gate. Got: {json}"
    );
    assert!(
        json.contains("403"),
        "Serialized Denial should contain code. Got: {json}"
    );
