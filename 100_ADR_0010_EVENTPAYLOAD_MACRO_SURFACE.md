...
(see `bpk-lib/crates/core/src/event/kind.rs`). Callers had to construct and pass that value at
...
- **Downstream crates** — via the root re-export
  `pub use batpak_macros::EventPayload;` at `bpk-lib/crates/core/src/lib.rs`, which makes
  ...
  `bpk-lib/crates/core/fixtures/downstream/` proves this.
- **Inside the `batpak` crate itself** — `pub extern crate self as batpak;`
  at the crate root makes `::batpak::...` resolve to `self::...` from within
  the crate, so `#[derive(EventPayload)]` inside `bpk-lib/crates/core/src/`-style modules works
...
**Authoritative list (verified against `bpk-lib/crates/core/src/store/mod.rs`):**
...
All four have shipped. See `bpk-lib/crates/core/tests/event_payload_surface.rs` for the positive
surface, `bpk-lib/crates/core/tests/derive_eventpayload_errors.rs` + `bpk-lib/crates/core/tests/ui/ep_*.{rs,stderr}`
for span-pointed compile errors, `bpk-lib/crates/core/fixtures/downstream/` for the
path-hygiene fixture, and `bpk-lib/crates/core/fixtures/kind-collision-composer/` for cross-crate
collision detection.
...
smoke test (`bpk-lib/crates/core/tests/event_payload_surface.rs`), the in-crate derive resolver
test, and the downstream fixture at `bpk-lib/crates/core/fixtures/downstream/`.
